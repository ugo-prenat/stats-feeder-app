...
const createBot = (req: Request, res: Response, next: NextFunction) => {
  const { streamer, name, username } = req.body;
  const bot = new Bot({
  ...
  return bot.save()
  .then(bot => res.status(201).json({ data: bot }))
...
const getBot = (req: Request, res: Response, next: NextFunction) => {
  ...
  .populate("streamer")
  .then(bot => bot ? res.status(200).json({ data: bot }) : res.status(404).json({ message: "Bot not found" }))
  .catch(error => res.status(500).json({ error }))
}
const getAllBot = (req: Request, res: Response, next: NextFunction) => {
  return Bot.find()
  .populate("streamer")
  .then(bots => res.status(200).json({ data: bots }))
  .catch(error => res.status(500).json({ error }))
}
