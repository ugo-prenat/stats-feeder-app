...
const getTweet = (req: Request, res: Response) => {
  ...
  return Tweet.findById(id)
    .populate('bot')
    .then(tweet =>
      tweet ? res.status(200).json({ tweet }) : res.status(404).json({ message: 'Tweet not found' })
    )
    .catch(error => res.status(500).json({ error }))
}
const getAllTweet = (req: Request, res: Response) => {
  return Tweet.find()
    .populate('bot')
    .then(tweets => res.status(200).json({ tweets }))
    .catch(error => res.status(500).json({ error }))
}
const updateTweet = (req: Request, res: Response) => {
  const { id } = req.params
